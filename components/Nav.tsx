    ...
    <header className="pointer-events-none fixed inset-x-0 top-0 z-50 flex justify-center">
      <motion.nav
        ...
        className={`pointer-events-auto mt-5 flex w-[min(92vw,1100px)] items-center justify-between rounded-full border py-2.5 pl-5 pr-2.5 transition-all duration-500 ease-editorial ${
          scrolled || open
            ? "border-line bg-paper/70 shadow-soft backdrop-blur-xl"
            : "border-transparent bg-paper/0"
        }`}
      >
        {/* Wordmark */}
        <a href="#main" className="group flex items-center gap-3" aria-label={profile.name}>
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            exit={{ opacity: 0 }}
            transition={{ duration: 0.4, ease: [0.16, 1, 0.3, 1] }}
            className="pointer-events-auto fixed inset-0 z-40 flex flex-col justify-center bg-paper/85 px-8 backdrop-blur-3xl md:hidden"
          >
            <nav className="flex flex-col gap-1">
              {navLinks.map((link, i) => (
                <motion.a
                  ...
                  className="border-b border-line py-4 font-display text-4xl font-medium tracking-tightest text-ink transition-colors duration-300 hover:text-brass-deep"
                >
